...
- [ ] 1.2 `docs/schema-v1.md`: the nine rules with why-clauses; `acChanged`; `source` on link events; arch enum; `tb.route`; `host.id` and `node.serialHash` (proposed rows already present) — issue #27
- [ ] 1.2b Both producers emit `host.id` (hashed platform UUID) and `serialHash` where a serial exists; dashboard keys hosts on `host.id` with `host.name` fallback
