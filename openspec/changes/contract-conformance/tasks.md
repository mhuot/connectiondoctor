...
- [ ] 1.2c Share-scoped redaction: `--redact` on `contract`/`report`/`diff` (implicit one-document scope), `bundle <out.zip> [--hours N] [--scope token]` (one explicit scope for envelope + report + events); pseudonymous `host.id` = HMAC(real id, scope token); recursive removal of `platform{}`, `unitKey`, serials incl. embedded `fullSnapshot` envelopes; tests: rename still joins locally, documents in one bundle share a `host.id`, two bundles do not correlate, no native id survives anywhere in a redacted bundle
