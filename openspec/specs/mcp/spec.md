...
## Purpose
The MCP door: both binaries serve the same read-only, stdio-only tool set (`connection_*`) with identical metadata from `docs/mcp-tools.json`, returning Contract v1 documents. Local to the machine by design — not a fleet transport (see `docs/architecture.md` § Fleet integration).
## Requirements
