...
| `builtIn` *(opt, proposed)* | `true` for integrated devices — internal keyboard/trackpad/camera, the built-in display and its hub — as classified by the producer. Consumers treat absence as unknown, never infer it from names, and filter as a view choice (`dashboard-topology-controls`, issues #42 #43) |
