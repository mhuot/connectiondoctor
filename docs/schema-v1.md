...
| `host.id` *(opt)* | **Opaque, random, per-installation** endpoint identity (UUIDv4 generated on first run, persisted in the data directory). Survives hostname changes and normal upgrades/reinstalls (which keep the data directory); regenerates only when identity state or the data directory is reset. **Not derived from hardware** — never a hash of IOPlatformUUID / MachineGuid, which would be a global tracking identifier. Consumers key hosts on it when present, `host.name` otherwise. Portable exports replace it with a **share-scoped** pseudonym (see § Redaction and share scope) so two shared bundles are not linkable while documents inside one bundle still join. Managed-fleet correlation uses a platform-supplied endpoint ID or a tenant-keyed HMAC — fleet-integration milestone, not this field. (issue #27) |
| `displaysKnown` | `false` when the producer had no display session (SSH on macOS); distinct from "no displays attached" |
| `findings` / `incidents` / `analysis` *(opt)* | `analysis: {windowHours, generatedAt, coverage, baseline?, capabilities?}` plus the two arrays. **Present whenever the producer has something to say** — recorded history, a live finding (a power deficit in front of the user), or a baseline it could not evaluate. **Absent** only when there is nothing at all: no history, no live finding, and a baseline that was readably absent. Absent ≠ empty: an absent field is "not reported", an empty array is "looked, found none". `analysis` without history is normal — `coverage` then carries `no-history` and says which temporal claims are impossible, while `findings` and `baseline` still describe the present. **`baseline`** = `{state: "no-baseline" \| "healthy" \| "active-fault" \| "recovered", capturedAt?, faultSince?, recoveredAt?}` — the known-good comparison state (`recovered` = recovered since a fault; one vocabulary everywhere). **Absent** when the producer could not evaluate it — the baseline was being replaced, or its file or fault history could not be read or written — with the reason in **`capabilities.baseline`** (`busy` | `unreadable` | `history-unreadable` | `history-unwritable`; `available` otherwise). Absent is unknown, never healthy. Baseline availability is deliberately *not* a coverage reason: coverage answers whether the recording covers the window, and "history complete, baseline unknown" is a real and sayable state. **`capabilities`** = `{linkEvents: "kernel" \| "notification" \| "poll" \| "unavailable", baseline?: "available" \| "busy" \| "unreadable" \| "history-unreadable" \| "history-unwritable"}` — what the collector can attribute or evaluate, a separate axis from temporal coverage: a gap-free history with ETW disabled is *complete* and *link-events unavailable*, not incomplete; likewise a complete window whose baseline could not be read is complete-with-unknown-baseline. **`coverage`** = `{availableFrom, through, complete: bool, reasons?: string[]}` — what the recorder can actually vouch for: `complete` is true only when the recording spans the whole requested window with no trim inside it and no gap longer than 3× the sample interval; `reasons` names why not. Two families, both about *the evidence for this window* and never about what a collector can attribute (that is `capabilities`):

- **temporal** — the recording does not span the window: `no-history`, `recorder-started-inside-window`, `recorder-stopped-before-window` (it ran, but not inside this window; `availableFrom`/`through` then point at the last evidence), `gap`, `trimmed`;
- **integrity** — evidence exists but cannot be trusted or read: `corrupt-lines`, `gap-evidence-unreadable`, `heartbeat-unreadable`, `trim-evidence-unreadable`, `no-heartbeat`.

The list is the canonical set at v1 and is **extensible**: producers may add reasons for evidence problems this list does not name, and consumers MUST treat an unknown reason as "incomplete for a reason I do not recognise" — displaying it verbatim — rather than rejecting the envelope or ignoring the incompleteness. What consumers may rely on is `complete` itself. Consumers show "unknown" rather than "none" whenever `complete` is false — an empty valid stream, a newly installed recorder and a trimmed log are indistinguishable without this |
...
| `builtIn` *(opt)* | `true` for integrated devices — internal keyboard/trackpad/camera, the built-in display and its hub — as classified by the producer. Consumers treat absence as unknown, never infer it from names, and filter as a view choice (`dashboard-topology-controls`, issues #42 #43) |
...
| `unitKey` *(opt)* | Distinguishes two units of the same VID:PID **within one collector's data**: `HMAC-SHA256("USB|<VID:PID>|<serial>", installationKey)` truncated to 16 hex chars, where `installationKey` is a random secret stored beside `host.id`. Keyed per installation, so it is not linkable across machines or exports and does not expose the serial (a plain serial hash is neither redaction nor safe for enumerable serials). The raw serial never leaves the machine. **The model is part of the hashed input**, because serials alone are not unique across products: placeholder and version-shaped tokens (`0001`, `1.00`, `0`) are common, and a key shared by a dock and a webcam would break the one thing this field promises. **The limit that remains**, stated rather than implied: a manufacturer that ships the same serial across every unit of one product leaves those units genuinely indistinguishable from outside, and no keying scheme invents the difference — equal keys mean equal units *as far as the device reports*. Cross-endpoint unit correlation ("the same bad dock following users") is a fleet-integration concern with a tenant-scoped key. (issue #27) |
